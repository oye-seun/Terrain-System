...
public class IslandMeshGen : MonoBehaviour
{
    ...
    public void GenerateMesh()
    {
        ...
        List<int> triangles2 = delauney2.Triangulate(false);
        ...
        int lastVert = _cutoutTopVerts.Count;
        //Debug.Log("lastvert: " + lastVert);
        int lastButtVert = _cutoutDownVerts.Count;


        // join both boundaries
        List<Vector3> uvSeamVerts;
        Dictionary<int, int> vertMap;
        List<int> boundarySeam = JoinMeshes(triangles, triangles2, lastVert, out uvSeamVerts, out vertMap);
    ...
    //private Vector3 GetExtremeVerticePos(IEnumerable<Vector3> boundary, bool positive)
    //{
    //    Vector3 extreme = Vector3.zero;
    //    if (positive)
    //    {
    //        extreme = new Vector3(float.MinValue, float.MinValue, float.MinValue);
    //        foreach (Vector3 vec in boundary)
    //        {
    //            if (vec.x > extreme.x) extreme.x = vec.x;
    //            if (vec.y > extreme.y) extreme.y = vec.y;
    //            if (vec.z > extreme.z) extreme.z = vec.z;
    //        }
    //    }
    //    else
    //    {
    //        extreme = new Vector3(float.MaxValue, float.MaxValue, float.MaxValue);
    //        foreach (Vector3 vec in boundary)
    //        {
    //            if (vec.x < extreme.x) extreme.x = vec.x;
    //            if (vec.y < extreme.y) extreme.y = vec.y;
